...
再來看一個例子，假設我們需要在一個長度為 `n` 的 `array` 中找一個元素，我們可以把查詢一次（執行一次操作）定義為 O(1)，如果我們知道該元素的 index，只需要查詢一次就可以得到答案，時間複雜度就是 O(1)，如果我們並不知道該元素的 index，則需要遍歷 `array` 才能找到答案，那最差情況下我們需要查詢 `n` 次，則時間複雜度為 O(n)。
